...
public class MimeTypeServiceImpl implements MimeTypeService, MimeTypesUpdatedListener {
  ...
  public void setInstitutionService(InstitutionService service) {
    mimeCache =
        service.newInstitutionAwareCache(
            new CacheLoader<Institution, EntryCache>() {
              @Override
  ...
  private List<MimeEntry> getNonInstitutionEntries() {
    return MimeFileUtils.readLegacyFile(
        getClass().getResourceAsStream("mimenoninst.types"), null); // $NON-NLS-1$
  }
  ...
  private void validate(MimeEntry mimeEntry) {
    ...
    if (!extensions.isEmpty()) {
      ...
      if (sz > 1 || (sz == 1 && entries.get(0).getId() != mimeEntry.getId())) {
        throw new InvalidDataException(
            new ValidationError("extensions", null, KEY_PFX + "error.extensions.alreadyinuse"));
      }
    }
  }
  ...
  public void setPluginService(PluginService pluginService) {
    textExtracterTracker =
        new PluginTracker<TextExtracterExtension>(
            pluginService,
            "com.tle.core.mimetypes", //$NON-NLS-1$
            "textExtracter",
            "id",
            new ExtensionParamComparator()); //$NON-NLS-1$ //$NON-NLS-2$
    textExtracterTracker.setBeanKey("class"); // $NON-NLS-1$

    attachmentResources =
        new PluginTracker<RegisterMimeTypeExtension<Attachment>>(
            pluginService,
            "com.tle.core.mimetypes",
            "attachmentResourceMimeType",
            null, //$NON-NLS-1$
