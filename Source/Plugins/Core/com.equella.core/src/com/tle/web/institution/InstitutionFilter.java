...
public class InstitutionFilter extends OncePerRequestFilter implements InstitutionListener {
    ...
    if (institutions == null) {
      ...
      for (final InstitutionStatus instStatus : enabledInsts) {
        ...
        if (inst.isEnabled()
          ...
          pairedInsts.add(
              new Pair<String, InstitutionStatus>(processUrl(inst.getUrl()), instStatus));
        }
      }

      // Add the Institution Management site in too
      pairedInsts.add(
          new Pair<String, InstitutionStatus>(
              processUrl(adminUrl), new InstitutionStatus(Institution.FAKE, -1)));

      // Sort largest URLs first
      Collections.sort(
          pairedInsts,
          new Comparator<Pair<String, InstitutionStatus>>() {
            @Override
            public int compare(
                Pair<String, InstitutionStatus> o1, Pair<String, InstitutionStatus> o2) {
  ...
  private Pair<String, Pair<String, InstitutionStatus>> getInstitutionForUrl(
  ...
  protected FilterResult doFilterInternal(HttpServletRequest request, HttpServletResponse response)
    ...
    if (matchInstPair == null) {
      if (LOGGER.isDebugEnabled()) {
        LOGGER.debug(
            "No institutions were matched for request ["
                ...
                + Joiner.on("] and [")
                    .join(
                        Collections2.transform(
                            getInstitutions(),
                            new Function<Pair<String, ?>, String>() {
                              @Override
  ...
  private Set<String> getMatchStrings(HttpServletRequest request) throws MalformedURLException {
    ...
    // which is really bad!
    String requestURI =
        URLUtils.basicUrlDecode(request.getRequestURI().substring(context.length()));
    Set<String> matchStrings =
        Sets.newHashSet("://" + requestedUrl.getHost() + portPart + context + requestURI);
  ...
  private FilterResult doNormal(
      HttpServletRequest request,
      HttpServletResponse response,
      Pair<String, Pair<String, InstitutionStatus>> matchInstPair)
      throws IOException {
